...
x_re = vae.predict(x_train)
print(x_train.shape, x_re.shape)

recons_loss = np.sum( (x_train-x_re) ** 2 ) / (32*32*3*40000)
print(recons_loss)

num = 32
for i in range(num):
    name = 'img_p2c/{}.png'.format(i)
    io.imsave(name,np.reshape(x_re[i,:],(32,32,3)) )
